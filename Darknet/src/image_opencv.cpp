...
// Draw Detection
// ====================================================================
extern "C" void draw_detections_cv_v3(mat_cv* mat, detection *dets, int num, float thresh, char **names, image **alphabet, int classes, int ext_output, int framenum)
{
    try {
        ...
        for (i = 0; i < num; ++i) {
            ...
            if (class_id >= 0) {
                ...
                if (dets[i].prob[0]>=thresh){
                    std::ofstream bboxout;
                    bboxout.open ("det.txt",ios::app|ios::ate);
                    if (bboxout.is_open()){
                        bboxout << framenum << ", -1, " << (round(left/2))*2 << ", " << top << ", " << right - left << ", "<< bot - top << ", "<< std::fixed << std::setprecision(5) << dets[i].prob[0]*100 << ",0,-1,-1" << endl;
                        bboxout.close();          //rounding encodes class; even numbers = class_id = 0 = baboons
                    }
                    else{
                        printf("Could not write to file");
                    }
                }
                if (dets[i].prob[1]>=thresh){
                    std::ofstream bboxout;
                    bboxout.open ("det.txt",ios::app|ios::ate);
                    if (bboxout.is_open()){
                        bboxout << framenum << ", -1, " << ((round(left/2))*2)-1 << ", " << top << ", " << right - left << ", "<< bot - top << ", "<< std::fixed << std::setprecision(5) << dets[i].prob[1]*100 << ",1,-1,-1" << endl;
                        bboxout.close();           //odd numbers class id = 1 buckets
                    }
                    else{
                        printf("Could not write to file");
                    }
    ...
    float precision, int draw_precision, char *accuracy_name, float contr_acc, int dont_show, int mjpeg_port, double time_remaining)
{
    try {
        ...
        if (k == 's' || current_batch == (max_batches - 1) || (current_batch / 100 > old_batch / 100)) {
            ...
            save_mat_png(img, "../gdrive/MyDrive/FYP/Charts/chart.png");
