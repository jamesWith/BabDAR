...
def Run_detection(crop, action_label):
	...
	if len(frames) == args.delta*6:	
		...
		#print('<----------------->')
		frames = []
		return detected_action, scores_indcies[:2], final_scores
		...
		with open(detfile[:-9] + "actionfinal.txt", 'w') as out_file:
			...
			for action in action_dets:
				if (action[1] != '-1') and ((action[0] not in baboonprevbucket) or (baboonprevbucket[action[0]]!=bucketdict[int(action[1])])):
					...
					print(bucketdict[int(action[1])].split()[0] + ',' + action[0]+ ','+ bucketcolourdict[bucketdict[int(action[1])].split()[1]] + ',' + str(math.floor(int(float(action[2]))/60))+':'+str(int(float(action[2]))%60) + ',' + str(baboonvisitnumber[action[0]]), file=out_file)
