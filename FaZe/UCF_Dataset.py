...
class TSNDataset(data.Dataset):
    ...
    def __init__(self, file_list,num_segments=3, new_length=1, modality='RGB',
                 image_prefix='{:06d}.jpg', transform=None, test_mode=False, train_val_switch=True):
    ...
    def Vid2Frames(self, info, indices):
        ...
        for seg_ind in indices:
            ...
            for i in range(self.new_length):
                seg_imgs = [Image.open(os.path.join('/content/gdrive/MyDrive/FYP/FaZe/',info.path, self.image_prefix.format(p))).convert('RGB')]
